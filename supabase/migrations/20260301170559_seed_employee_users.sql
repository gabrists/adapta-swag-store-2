...
DECLARE
  ...
  users_to_insert JSON := '[
    {"name": "Márcia Ertel", "email": "marcia@adapta.org"},
    {"name": "Kimberly Prestes", "email": "kimberly@adapta.org"},
    {"name": "Juan Bonfim", "email": "juan@copyexperts.com.br"},
    {"name": "Mateus Sotto", "email": "matheus.sotto@adapta.org"},
    {"name": "Felipe Navaar", "email": "navaar@adapta.org"},
    {"name": "Felipe Oliveira Garcia", "email": "felipe.garcia@adapta.org"},
    {"name": "Edson Muniz", "email": "edson@adapta.org"},
    {"name": "Jadson Consolini", "email": "jadson@copyexperts.com.br"},
    {"name": "Ruan Azevedo", "email": "ruan.azevedo@adapta.org"},
    {"name": "Lucas Bueno", "email": "bueno@adapta.org"},
    {"name": "Mateus Tápias", "email": "mateustapias2015@gmail.com"},
    {"name": "Thiago Machado", "email": "contatocostathiago@gmail.com"}
  ]';
BEGIN
  ...
  LOOP
    ...
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = user_info.email) THEN
      ...
      IF NOT EXISTS (SELECT 1 FROM public.employees WHERE email = user_info.email) THEN
        ...
        VALUES (new_user_id, user_info.name, user_info.email);
      END IF;
    END IF;
  END LOOP;
END $$;
